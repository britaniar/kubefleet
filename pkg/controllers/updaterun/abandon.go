...
func (r *Reconciler) abandon(
	...
	if updatingStageIndex < len(updateRunStatus.StagesStatus) {
		...
		finished, waitTime, execErr := r.abandonUpdatingStage(updateRun, updatingStageIndex, toBeUpdatedBindings, maxConcurrency)
	...
	// All the stages have finished, abandon the delete stage.
	finished, execErr := r.abandonDeleteStage(updateRun, toBeDeletedBindings)
	if errors.Is(execErr, errStagedUpdatedAborted) {
...
// abandonUpdatingStage abandons the updating stage by letting the updating bindings finish and not starting new updates.
func (r *Reconciler) abandonUpdatingStage(
	updateRun placementv1beta1.UpdateRunObj,
	updatingStageIndex int,
	...
	if clusterUpdatingCount == 0 && clusterUpdated {
		...
		klog.V(2).InfoS("The stage has finished all clusters updating", "stage", updatingStageStatus.StageName, "updateRun", updateRunRef)
		return true, 0, nil
	} else if clusterUpdatingCount == 0 && !clusterUpdated {
		// No clusters are updating or updated in this round, meaning all remaining clusters have not started yet or succeeded.
...
// abandonDeleteStage abandons the delete stage by letting the deleting bindings finish.
func (r *Reconciler) abandonDeleteStage(
	updateRun placementv1beta1.UpdateRunObj,
