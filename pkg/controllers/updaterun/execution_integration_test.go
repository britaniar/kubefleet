...
var _ = Describe("UpdateRun execution tests - double stages", func() {
	...
	Context("Cluster staged update run should finish current updating clusters when paused", Ordered, func() {
		var wantApprovalRequest *placementv1beta1.ClusterApprovalRequest
		...
		It("Should start pausing the update run when state is Pause", func() {
			By("Updating updateRun state to Pause")
			updateRun.Spec.State = placementv1beta1.StatePaused
			Expect(k8sClient.Update(ctx, updateRun)).Should(Succeed(), "failed to update the updateRun state")

			By("Validating update run is pausing")
			// Mark updateRun progressing condition as false with pausing reason.
			meta.SetStatusCondition(&wantStatus.StagesStatus[0].Conditions, generateFalseConditionWithReason(updateRun, placementv1beta1.StageUpdatingConditionProgressing, condition.StageUpdatingPausingReason))
			meta.SetStatusCondition(&wantStatus.Conditions, generateFalseConditionWithReason(updateRun, placementv1beta1.StagedUpdateRunConditionProgressing, condition.UpdateRunPausingReason))
			validateClusterStagedUpdateRunStatus(ctx, updateRun, wantStatus, "")

			By("Checking update run status metrics are emitted")
			wantMetrics = append(wantMetrics, generatePausingMetric(updateRun))
			validateUpdateRunMetricsEmitted(wantMetrics...)
		})

		It("Should mark the 2nd cluster in the 1st stage as succeeded after pausing the updateRun", func() {
			By("Validating the 2nd clusterResourceBinding is updated to Bound")
			binding := resourceBindings[numTargetClusters-3] // cluster-7
			validateBindingState(ctx, binding, resourceSnapshot.Name, updateRun, 0)
			...
			meta.SetStatusCondition(&wantStatus.StagesStatus[0].Conditions, generateFalseConditionWithReason(updateRun, placementv1beta1.StageUpdatingConditionProgressing, condition.StageUpdatingPausedReason))
			meta.SetStatusCondition(&wantStatus.Conditions, generateFalseConditionWithReason(updateRun, placementv1beta1.StagedUpdateRunConditionProgressing, condition.UpdateRunPausedReason))
			validateClusterStagedUpdateRunStatus(ctx, updateRun, wantStatus, "")
		...
		It("Should start executing the update run when state is Execute", func() {
			By("Updating updateRun state to Execute")
			updateRun.Spec.State = placementv1beta1.StateStarted
			Expect(k8sClient.Update(ctx, updateRun)).Should(Succeed(), "failed to update the updateRun state")

			By("Validating update run is execute")
			// Mark 3rd cluster as started.
			wantStatus.StagesStatus[0].Clusters[2].Conditions = append(wantStatus.StagesStatus[0].Clusters[2].Conditions, generateTrueCondition(updateRun, placementv1beta1.ClusterUpdatingConditionStarted))
			// Mark updateRun and stage progressing condition as true with progressing reason.
		...
		It("Should start pausing the update run when state is Pause", func() {
			...
			By("Validating update run is paused")
			// Mark updateRun progressing condition as false with paused reason.
			meta.SetStatusCondition(&wantStatus.StagesStatus[1].Conditions, generateFalseConditionWithReason(updateRun, placementv1beta1.StageUpdatingConditionProgressing, condition.StageUpdatingPausedReason))
			meta.SetStatusCondition(&wantStatus.Conditions, generateFalseConditionWithReason(updateRun, placementv1beta1.StagedUpdateRunConditionProgressing, condition.UpdateRunPausedReason))
			validateClusterStagedUpdateRunStatus(ctx, updateRun, wantStatus, "")
		...
		It("Should not continue rolling 2nd stage when paused", func() {
			By("Validating update run is paused")
			validateClusterStagedUpdateRunStatus(ctx, updateRun, wantStatus, "")
...
func labelPairsToMap(pairs []*promclient.LabelPair) prometheus.Labels {
	m := prometheus.Labels{}
	for _, p := range pairs {
...
func removeMetricFromMetricList(metricList []*promclient.Metric, metricToRemove *promclient.Metric) []*promclient.Metric {
	var result []*promclient.Metric
